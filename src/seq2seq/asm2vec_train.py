...
def main(Word2vec_train_set='Load', model_load=None, On_Service=False, FBH_mode=False):
    ...
    result_f.write('Begin the training based on Asm2vec function embedding technique.\n')

    train_comment_batch, train_embedding_batch, \
    val_comment_batch, val_embedding_batch, \
    test_comment_batch, test_embedding_batch, whole_comment_batch = prepare_data(path_prefix + data_path)
    ...
    if 'New' not in Word2vec_train_set and model_load is not None and \
        ...
        # temp_word2vec_train = padded_train_comment + padded_test_comment + padded_old_comment
...
if __name__ == '__main__':
    ...
    parser.add_argument('--FSE', '-f', required=False, action='store_true', help='Whether use Bin2Summary module.')
