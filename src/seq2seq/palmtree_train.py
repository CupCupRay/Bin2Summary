...
def prepare_data(path, my_seed=42):
    # train_com_batch = []  # Batches of comments, list [comment_1 <list<str>>, comment_2 <list<str>>, ...]
    # train_emb_batch = []  # Batches of embeddings, list [file_1 <list<list<float>>>, file_2 <list<list<float>>, ...]
    train_com_batch, train_emb_batch = dict(), dict()
    val_com_batch, val_emb_batch = dict(), dict()  # Batches of com/emb, dict [pack: list [com_1, com_2], ...]
    ...
    if not E_Exists or not C_Exists:
        print("No existing embeddings file. Begin to extract the features (been preprocessed).")
        for root, dirs, files in os.walk(path):
            if not root.endswith('/'): root = root + '/'
            # Clean the old files
            if root == path or root == path + '/':
        ...
        for root, dirs, files in os.walk(path):
            ...
            for file in files:
                if file.endswith('embeddings.record') or file.endswith('comment.record'):
                    ...
                    postfix = file[file.rfind('_'):]
                    with open(path + prefix + postfix, mode='a') as record_file:
    ...
    for p in packs:
        ...
        for com, emb in zip(com_file, emb_file):
            ...
            ori_emb_data = json.loads(emb)
            emb_data = ori_emb_data
            assert type(emb_data) == list
            for ele in emb_data:
                assert type(ele) == list
                assert len(ele) == BB_VECTOR_DIM
                for num in ele:
...
def preprocess_embeddings(embeddings, comment, padding=True):
    ...
    for emb in embeddings:
        for token in emb:
            if str(token) == 'nan' or str(token) == 'Nan' or str(token) == 'NaN' or \
                ...
                # print('WARNING! Find Nan or Inf, SKIP! Comment:', comment)
    ...
    if padding and BB_MAX_NUM is not None:
        if count > BB_MAX_NUM:
            ...
            # Directly add padding to the rest of the space
...
def main(Word2vec_train_set='Load', model_load=None, On_Service=False, data='PalmTree-data/'):
    ...
    result_f.write('Begin the training based on PalmTree function embedding technique.\n')
    result_f.write('Using the dataset in ' + data + '\n')

    train_comment_batch, train_embedding_batch, \
    val_comment_batch, val_embedding_batch, \
    test_comment_batch, test_embedding_batch, whole_comment_batch = prepare_data(path_prefix + data)
    ...
    # print(np.shape(train_embedding_batch))
    ...
    # Apply the Gensim.word2vec model
    ...
    if 'New' not in Word2vec_train_set and model_load is not None and \
        ...
        temp_word2vec_train = whole_comment_batch + padded_train_comment + padded_val_comment + padded_test_comment
    ...
    # val_result = my_model.evaluate_generator(val_dataset.make_one_shot_iterator())
    with open(path_prefix + 'results/' + timeRecord + '.log', mode='a') as result_f:
