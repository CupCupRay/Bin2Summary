...
def _new_train_vectorized(wnd: SequenceWindow, f: VectorizedFunction, context: TrainingContext) -> np.ndarray:
    ct_prev = _get_inst_repr(wnd.prev_ins_op(), wnd.prev_ins_args())
    ct_next = _get_inst_repr(wnd.next_ins_op(), wnd.next_ins_args())
    delta = np.average([ct_prev, f.v, ct_next], axis=0)
...
def _new_train_sequence(f: VectorizedFunction, seq: List[Instruction], context: TrainingContext) -> list:
    wnd = context.create_sequence_window(seq)
    ins_vectors = []
    while wnd.move_next():
        current_ins_vec = _new_train_vectorized(wnd, f, context)
        ins_vectors.append(current_ins_vec)
    return ins_vectors
...
def estimate(f: VectorizedFunction, estimate_repo: FunctionRepository, params: Asm2VecParams) -> np.ndarray:
    ...
    # Add functionality-related here
    for seq in f.sequential().sequences():
        _train_sequence(f, seq, context)

    return f.v

def new_estimate(f: VectorizedFunction, estimate_repo: FunctionRepository, params: Asm2VecParams) -> dict:
    context = TrainingContext(estimate_repo, params, True)
    # Add functionality-related here
    ins_vectors = dict()
    appearance = dict()
    for seq in f.sequential().sequences():
        temp_ins_vectors = _new_train_sequence(f, seq, context)
        assert len(seq[1:-1]) == len(temp_ins_vectors)
        for token, vec in zip(seq[1:-1], temp_ins_vectors):
            complete_ins = Instruction.op(token)
            if Instruction.args(token):
                complete_ins = complete_ins + ' ' + ', '.join(Instruction.args(token))
            if complete_ins not in ins_vectors:
                ins_vectors[complete_ins] = vec
                appearance[complete_ins] = 1
            elif complete_ins in ins_vectors:
                assert ins_vectors[complete_ins].shape == vec.shape
                temp_vec = ins_vectors[complete_ins]
                temp_vec = temp_vec + vec
                ins_vectors[complete_ins] = temp_vec
                assert ins_vectors[complete_ins].shape == vec.shape
                appearance[complete_ins] += 1

    for ele in ins_vectors:
        temp_vec = ins_vectors[ele]
        temp_vec = temp_vec / appearance[ele]
        ins_vectors[ele] = temp_vec
