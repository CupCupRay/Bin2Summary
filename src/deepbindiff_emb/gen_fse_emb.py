...
def CalInsEmbeddings(blockIdxToTokens, blockIdxToOpcodeNum, blockIdxToOpcodeCounts, insToBlockCounts,
                       tokenEmbeddings, reversed_dictionary, opcode_idx_list, FS_BlockIdx):
    ins_embeddings = {}
    totalBlockNum = len(blockIdxToOpcodeCounts)
    ...
    for bid in blockIdxToTokens:
        ...
        if len(tokenlist) != 0:
            ...
            if bid not in FS_BlockIdx:
                opcode_embed = opcode_embed.sum(0)
                operand_embed = operand_embed.sum(0)
        ...
        if bid not in FS_BlockIdx:
            block_embed = np.concatenate((opcode_embed, operand_embed), axis=0)
        else: block_embed = np.concatenate((opcode_embed, operand_embed), axis=1)
        ...
        for key in block_embeddings:
            ...
            if value.ndim == 1:
                ...
                for line in range(len(value)):
                    ...
                    counter = counter + 1
                    # print(key, counter)
            else: print('WARNING! value.ndim:', value.ndim)
...
def generate_features(filepath, filename, funcname_comment_dict):
    ...
    for line in open(filepath + filename + '.snippet', mode='r'):
        ...
        this_func_name = semantic_info.strip('_')
        print('Function:', this_func_name)

        my_comment = find_related_comment(funcname_comment_dict, this_func_name)
        ...
        # Step 3: Find functionality-specified blocks
        FBH_Block, FBH_Block_Idx = [], []
        if len(BToIdx) < MAX_DATAFLOW_BLOCK:
            FBH_Block = use_depgraph.depgraph_analysis(filepath + filename, this_func_name)
            for block in FBH_Block:
                if block in BToIdx:
                    FBH_Block_Idx.append(BToIdx[block])
            print('Finish the Step 3')
        else: print('Skip the Step 3 since this function have too many blocks:', len(BToIdx))
        print('Functionality blocks:', FBH_Block, FBH_Block_Idx)

        LOOP = DATA_EXPANDING
        for _ in range(LOOP):
            processed_comment = my_comment

            # Step 4: Generate random walks, each walk contains certain blocks
            ...
            # step 5: Generate articles based on random walks
            ...
            # step 6: Token embedding generation
            # with strategy.scope():
            ...
            # step 7: Calculate feature vector for instructions
            InsEmbeddings = CalInsEmbeddings(BIdxToTokens, BIdxToOpcodeNum, BIdxToOpcodeCounts, IToBlockCounts,
                                             TokenEmbeddings, Rever_TokenDict, OIdxList, FBH_Block_Idx)
            InsVecFileGen(filepath + filename + '.feature', InsEmbeddings)
            print('Finish the Step 7')
