...
def generate_batch(article, blockBoundaryIdx, insnStartingIndices, indexToCurrentInsnsStart, opcode_idx_list):
    ...
    for i in range(batch_size):
        ...
        if article_size != 1 and len(insnStartingIndices) > 1:
            ...
            if nextInsnStart != -1:
                if nextInsnStart == insnStartingIndices[-1]:
...
def cal_operand_embedding(insnToken_embeddings, insn_opcode):
    size = tf.cast(tf.size(insnToken_embeddings), dtype=tf.float32)
    operand_embedding = tf.subtract(tf.math.divide(tf.reduce_sum(insnToken_embeddings, 0), size), tf.math.divide(insn_opcode, size))
...
def buildAndTraining(article, blockBoundaryIndex, insnStartingIndices, indexToCurrentInsnsStart, dictionary, opcode_idx_list):
    ...
    with g.as_default():
        ...
        for i in range(batch_size):
            ...
            currInsn = tf.math.divide(tf.add(prevInsn_embedding, nextInsn_embedding), 2.0)
    ...
    # training
    config = tf.compat.v1.ConfigProto(device_count={"CPU": 32},
                                      inter_op_parallelism_threads=1,
                                      intra_op_parallelism_threads=16,
                                      log_device_placement=False)
    with tf.compat.v1.Session(graph=g, config=config) as session:
